"""
    ...
    This module is where we do webapp alerts, which are part of the larger
    admin module.
...
def get_webapp_alerts():
    """ Returns all webapp alerts as a Flask response with a JSON payload."""

    alerts = utils.mdb.notifications.find(
        {'type': 'webapp_alert', 'expired': False}
    ).sort('created_on', -1)

    alerts = list(alerts)
    return Response(
        response=json.dumps(alerts, default=json_util.default),
        status=200,
...
class Alert:
    ...
    def __init__(self, _id=None):
        """ Initialize a webapp alert object from an _id. If one is not
        supplied, create a new alert. """
    ...
    def load(self):
        ...
        if self.alert is None:
            raise utils.InvalidUsage(
                'Notification %s does not exist!' % (self._id),
                status_code=404
            )
    ...
    def serialize(self):
        """ Serialize the alert. """
        return Response(
            response=json.dumps(self.alert, default=json_util.default),
            status=200,
            mimetype="application/json"
        )
